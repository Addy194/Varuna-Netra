...
async def scene_quicklook(scene_id: str, user=Depends(get_current_user)):
    ...
    if not (scene.get("metadata") or {}).get("preview_href") and not scene.get("quicklook_path"):
        # Demo/manual scenes get a deterministic synthetic quicklook from the detector.
        pass
    try:
...
async def detect_scene_modern(scene_id: str, user=Depends(require_role("analyst"))):
    scene = await db.scenes.find_one({"id": scene_id}, {"_id": 0})
    if not scene: raise HTTPException(404, "scene not found")
    try: return clean(await detect_scene(scene, user["email"]))
    except Exception as e: raise HTTPException(502, f"detector failed: {str(e)[:200]}")
