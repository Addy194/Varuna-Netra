...
ANALYST = ("analyst@varunanetra.demo", os.environ.get("TEST_ANALYST_PASSWORD"))
SUPERVISOR = ("supervisor@varunanetra.demo", os.environ.get("TEST_SUPERVISOR_PASSWORD"))
...
class TestDarkVessel:
    def test_scan_on_sar_ml_detector_case(self):
        tok = _login(*ANALYST)
        cases = requests.get(f"{API}/cases", headers=_h(tok), timeout=30).json()
        target = next((c for c in cases if c.get("source") == "sar_ml_detector"), None)
        assert target, "no case with source=sar_ml_detector found"
        case_id = target["id"]
