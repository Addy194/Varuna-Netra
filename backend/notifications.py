...
def alert_html(alert: dict, case: dict) -> str:
    e = html.escape
    link = f"{os.environ.get('FRONTEND_URL', 'http://localhost:3000').rstrip('/')}/cases/{alert['case_id']}"
...
async def notify_alert(alert: dict, case: dict) -> dict:
    ...
    subject = f"[VarunaNetra] {alert['severity'].upper()} alert — {case['case_number']}" + (f" · {case['icg']['code']}" if case.get("icg") else "")
    if not cfg["enabled"] or not cfg.get("alerts_enabled", True):
        summary = {"status": "disabled", "recipients": to, "sent": 0, "failed": 0, "at": now, "reason": "email alerts disabled"}
    elif not to:
        ...
        status="sent" if sent == len(to) else ("partial" if sent else "failed")
        summary = {"status": status, "recipients": to, "sent": sent, "failed": len(to) - sent, "at": now, "mode": modes[0] if len(modes)==1 else modes,
