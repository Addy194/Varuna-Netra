...
SUP = ("supervisor@varunanetra.demo", os.environ["TEST_SUPERVISOR_PASSWORD"])
ANL = ("analyst@varunanetra.demo", os.environ["TEST_ANALYST_PASSWORD"])
...
class TestDetector:
    ...
    def test_detect(self, tok_analyst, scene_with_preview):
        ...
        d = r.json()
        assert d.get("operational_validation_required") is True
        assert d.get("detector") == "sar-ml-pixel-1.0.0"
        assert "spots" in d and "cases" in d
        if d["cases"]:
            case_summary = d["cases"][0]
            cid = case_summary["case_id"]
            assert 0 <= (case_summary.get("confidence") or 0) <= 1
            # fetch full case doc
            rc = requests.get(f"{API}/cases/{cid}", headers=_h(tok_analyst), timeout=30)
            assert rc.status_code == 200
            case = rc.json()
            assert case.get("source") == "sar_ml_detector"
            assert "ml_candidate" in (case.get("quality_flags") or [])
            # check attachments
...
class TestRegression:
    ...
    def test_evidence_pdf_dark_spot(self, tok_analyst):
        r = requests.get(f"{API}/cases", headers=_h(tok_analyst), timeout=30)
        ds = next((c for c in r.json() if c.get("source") == "sar_ml_detector"), None)
        if not ds:
