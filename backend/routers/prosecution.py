...
async def verify_hash(hash_value: str):
    ...
    if not rec:
        return {"verified": False, "status": "unknown_hash", "message": "No prosecution export with this hash is recorded in the VarunaNetra audit ledger."}
...
async def verify_upload(file: UploadFile = File(...)):
    ...
    try:
        ...
        return {"verified": False, "status": "invalid_bundle", "bundle_sha256": bundle_hash, "message": "Not a VarunaNetra bundle (MANIFEST.json missing or unreadable)."}
    file_checks = []
