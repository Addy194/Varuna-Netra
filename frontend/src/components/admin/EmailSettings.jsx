  ...
  return (
    <div className="panel p-5 fade-up" data-testid="email-settings">
      <div className="mb-3 flex items-center gap-2"><Mail size={16} color="#00F0FF" /><h2 className="font-display text-lg font-semibold">Email delivery (Resend)</h2>
        <span data-testid="email-configured-badge" className="ml-auto rounded-full px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider" style={{ color: s.configured ? "#10B981" : "#FFB703", border: `1px solid ${s.configured ? "#10B981" : "#FFB703"}66` }}>{s.mode === "demo" ? "demo outbox" : s.configured ? "configured" : "not configured"}</span></div>
      <p className="mb-3 text-xs text-slate-400">{s.mode === "demo" ? "Demo mode records notification emails in a MongoDB outbox so the complete workflow can be demonstrated without credentials. Add a Resend key to switch to live delivery." : "Password-reset links and alert notifications are sent through Resend. Verify a sender domain before production use."}</p>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <button data-testid="btn-test-email" disabled={busy} onClick={test} className="inline-flex items-center gap-1.5 rounded border px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider text-slate-300 hover:text-white disabled:opacity-50" style={bd}><Send size={12} /> Send test to me</button>
