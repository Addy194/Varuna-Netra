  ...
  return (
    <nav className="flex flex-1 flex-col gap-4 overflow-y-auto overflow-x-hidden px-2 py-3 [&::-webkit-scrollbar]:w-1.5" data-testid="sidebar-nav">
      ...
      <div className="mt-auto flex flex-col gap-1 border-t pt-3" style={{ borderColor: "var(--border-default)" }}>
        {!collapsed && <p className="label-mono px-3 pb-1 text-[9px] text-slate-600">SIH</p>}
        <NavLink to="/demo" data-testid="nav-demo-link" onClick={onNavigate} title={collapsed ? "Run SIH Demo" : undefined}
          className={`${itemBase} ${collapsed ? "justify-center px-0" : "gap-3 px-3"} bg-cyan-400/15 font-semibold text-cyan-300 ring-1 ring-cyan-400/40 hover:bg-cyan-400/25`}>
          <Play size={16} className="shrink-0" />
          {!collapsed && <span className="truncate">Run SIH Demo</span>}
          {collapsed && <Tip label="Run SIH Demo" />}
        </NavLink>
        <div title={collapsed ? (refPinned ? `Reference case pinned: ${ref?.case_number || ""}` : "Reference case not pinned") : undefined}
