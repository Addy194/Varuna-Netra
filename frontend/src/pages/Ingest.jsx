...
const SceneForm = ({ onDone }) => {
  const [f, setF] = useState({ provider: "sentinel-1", provider_scene_id: `S1A_IW_GRDH_${Date.now().toString().slice(-6)}`, sensor_mode: "IW", polarization: "VV+VH", acquisition_time: "2026-06-12T06:15:00Z", storage_ref: "s3://varunanetra/scenes/new.tiff", footprint: SAMPLE_FOOTPRINT });
  const [busy, setBusy] = useState(false);
  const submit = async () => {
    setBusy(true);
