...
async def seed_demo():
    ...
    scene1 = await create_scene(SceneCreate(
        ...
        storage_ref="s3://varunanetra-demo/scenes/S1A_20260610T053000.tiff", metadata={"orbit": "descending", "relative_orbit": 37}), "seed")
    scene2 = await create_scene(SceneCreate(
        ...
        storage_ref="s3://varunanetra-demo/scenes/S1B_20260611T174500.tiff", metadata={"orbit": "ascending", "note": "no external polygon — run the bundled ML detector on the offline demo scene"}), "seed")
    ...
    ml_result = await detect_scene(scene2, "seed")
    ml_cases = ml_result.get("cases", [])
    for ml_case in ml_cases[:3]:
        job = await enqueue("correlate", {"case_id": ml_case["id"], "params": None}, "seed", inline=True)
        await process(job["id"])
    return {"seeded": True, "scenes": 2, "cases": 3 + len(ml_cases), "ml_detector_cases": len(ml_cases), "ais": summary}
