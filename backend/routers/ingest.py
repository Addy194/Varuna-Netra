...
async def detect_scene(scene_id: str, correlate: bool = True, user=Depends(require_role("analyst"))):
    ...
    result = await detect_scene(scene, user["email"])
    spills = await db.spill_observations.find({"scene_id": scene_id}, {"_id": 0}).sort("created_at", -1).to_list(20)
    case = result["cases"][0] if result.get("cases") else None
    spill = spills[0] if spills else None
    job = await enqueue("correlate", {"case_id": case["id"], "params": None}, user["email"]) if correlate and case else None
    return clean({"spill_observation": spill, "case": case, "job": job, "detection": result})
